...
throw new IllegalArgumentException ( lr_2 + fields ) ;
}
try {
...
throw new IllegalArgumentException ( lr_3 + fields ) ;
}
if ( datePresent ) {
bld . appendLiteral ( 'T' ) ;
}
...
throw new IllegalArgumentException ( lr_4 + fields ) ;
}
if ( ! hour && ( minute && second || ( minute && ! milli ) || second ) ) {
} else {
if ( strictISO ) {
throw new IllegalArgumentException ( lr_5 + fields ) ;
}
}
}
if ( hour ) {
...
. appendTimeZoneOffset ( lr_6 , false , 2 , 2 )
. toFormatter () ;
}
return bt ;
}
...
if ( ze == null ) {
return new DateTimeFormatterBuilder ()
. appendTimeZoneOffset ( lr_6 , true , 2 , 4 )
. toFormatter () ;
}
return ze ;
}
